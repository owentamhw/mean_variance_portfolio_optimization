...
stock_prices = getdata(tickers=tickets, start=datetime(2014,1,1), end=datetime(2018,1,1), to_csv=False)
...
# df with simple returns excluding the column 'Date'
...
df_random['Sharpe Ratio'] = (df_random['Returns'] - risk_free_rate) / df_random['Volatility']
...
ef_vol, ef_ret, ef_w, opt_max_sharpe, opt_max_sharpe_w = efficient_frontier(returns=returns, risk_free_rate=risk_free_rate)
opt_min_vol_pos = ef_vol.index(min(ef_vol))
opt_max_sharpe_ret = np.dot(opt_max_sharpe_w, asset_mean)
opt_max_sharpe_vol = np.sqrt(np.dot(opt_max_sharpe_w, np.dot(opt_max_sharpe_w, returns.cov())))

#####################################################
# Drawing
df_random.plot.scatter(x='Volatility', y='Returns', fontsize=12)

plt.scatter(max_sharpe_vol,max_sharpe_ret,marker='^',color='r',s=150, label='Rand Port w/ Maximum Sharpe ratio: '+ str(round(df_random['Sharpe Ratio'][max_sharpe_pos], 4)))
plt.scatter(min_vol_vol,min_vol_ret,marker='x',color='r',s=150, label='Rand Port w/ Minimum volatility') 

plt.plot(ef_vol, ef_ret, 'k-', markersize=2)
plt.scatter(opt_max_sharpe_vol, opt_max_sharpe_ret, marker='^',color='g',s=150, label='Maximum Sharpe ratio: '+ str(round(opt_max_sharpe, 4)))
plt.scatter(ef_vol[opt_min_vol_pos], ef_ret[opt_min_vol_pos], marker='x',color='g',s=150, label='Minimum volatility')

plt.scatter(asset_std, asset_mean, marker='+',color='black',s=250)
for i, s in enumerate(returns.columns):
  plt.annotate(s, (asset_std[i], asset_mean[i]), xytext=(2,5), textcoords='offset points')
